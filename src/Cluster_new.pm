...
print STDERR "\$this->{'Seg'}->{'Eng_Seg_Phrase'}=($this->{'Seg'})->$this->{'Seg'}->{'Eng_Seg_Phrase'}\n";
    }
    return $old;
}
...
sub InitDocSrc {
    my($me, $Src, $FileList, $sql) = @_; my(@FileList);
    $me->{'Src'} = $Src; 
    $me->{'DocIndex'} = 0;
    if ($Src eq 'Dir') {
...
# $sql = "select ID, SNo, Fname, Dname, Desc from $rDC->{Table} where ID = ?";
    $DBH->{LongReadLen}=1280000; # only work for SELECT, not for INSERT
    $DBH->{LongTruncOk} = 0;
    $STH = $DBH->prepare($sql)
             or die "Can't prepare SQL statement: $DBI::errstr\n";
...
# to consider short phrases as phrases, # 2006/04/16
...
=cut
sub SaveDocIndex {
    my($me) = @_;  my($InvF, $TiF, $DPF, $term, $i, $ptg);
...
# next line is added on 2005/08/15 for Cosine similarity
	foreach $d (@Docs) { $DocTermNum[$d] += (log(1+$PTG{$d})*$tlen*$idf)**2; }
    }
	for ($i=0; $i<$df; $i++) {
	    for($j=$i+1; $j<$df; $j++) {
	    	$f = ($Docs[$i]<$Docs[$j])?
...
print STDERR "\$Sim{$f}=",$me->ts($Sim{$f})," : tlen=$tlen, $PTG{$Docs[$i]}+$PTG{$Docs[$j]}, ",
...
print STDERR "  It takes ", time()-$stime, " seconds\n" if $me->{debug} >= 1;
    $stime = time();
# Now we have %Sim and @DocTermNum, we can compute Dice coefficient
print STDERR "Step 2: Calculate Dice coefficients between document pairs.\n"if $me->{debug} >= 1;
...
#    $NTi = $percent  = 0; 
    ...
    foreach $d (@Docs) {
#    	($i, $j) = split /:/, $d;
    	$f = sprintf("%1.4f", $Sim{$d}); # 2007/08/08, change 1.6f into 1.4f
...
sub ts { 
    my($me, $x, $n) = @_; 
    $n=4 if not $n; return sprintf("%0.".$n."f", $x); # 2007/08/08 change $n=5 into $n=4
#    return sprintf("%0.". ($n>0?$n:$me->{"DigitNum"}) ."f", $x); 
}
...
sub CutCollection {
    my($me, $Threshold) = @_; 
    my($root, @NewRoot, $rTree, $rRoot, %CatId2DocIds, %DocId2CatIds);
    my(@DocList, $did, $cid);
print STDERR "\nPartition documents with threshold: '$Threshold'.\n" if $me->{debug}>=1;
	...
	$CatId2DocIds{(split/:/,$root)[0]} 
		= &GetCat2Docs($root, $rTree, \%CatId2DocIds);
...
# output to a browser
# Use attributes: 'Title', 'Cid2Terms', 'FileList', 'ShowDoc'
...
#    $htmlstr .= "<li>$level$i. $node\n";
	...
	for ($j=0; $j<@Sons; $j++) {
        ...
        $h .= "</UL>\n" if @Sons > 1;
	$htmlstr .= $me->ClusterTitleString($node, $rCid2Terms, 
		$ctt, $NumRecords) . $h;
    }
...
sub CreateBigDoc {
    ...
    my($rCid2Terms, $ctt, $str, $NumRecords);
    ...
    foreach $root (@Root) { # get each new root's number of leaves
    	...
    	$title =~ s#<[^>]+>|\n+##g;
    	$str = $me->DumpContent($file, @DocList);
...
#   return this string
sub DumpContent {
    my($me, $file, @DocList) = @_;
    return $me->DumpContentFromDBMS($file, @DocList) if $me->{DSN};
...
sub DumpContentFromDBMS {
    my($me, $file, @DocList) = @_;  
    my($DSN, $sql, $DBH, $STH, $id, $title, $content, $str, $did);
    my $rDocPath = $me->{'DocPath'};
    $DSN = $me->{DSN}; $sql = $me->{SQL};
...
print STDERR "Computing Correlation(t, c) for $tk pairs...\n" if $me->{debug} >= 1;
    while (($k, $tp) = each %$rTP) {
	$ti++;	$percent = $Progress->ShowProgress($ti/$tk, $percent) if $me->{debug} >= 1;
    	($t, $c) = split /\t/, $k;
    	$fp = $rDF->{$t} - $tp;
    	$fn = $rDFcat->{$c} - $tp;
