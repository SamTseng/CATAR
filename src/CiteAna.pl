...
# option OmaxDF can be used to exclude those references that are commonly cited.
...
sub GetCitations { 
	...
	while (($id, $CR, $nr) = $STH->fetchrow_array) {
		$encoding_name = Encode::Detect::Detector::detect($CR);
		if ($encoding_name =~ /UTF-8/i) { # if utf8-encoded
#			$CR = encode("big5", $CR); # from utf-8 to big5
#		if ($encoding_name !~ /big5/i) { # if utf8-encoded
			from_to($CR, $encoding_name, 'big5'); 
		}		
		@CR = split /;\s*/, $CR;
		$idn++;
		push @List, "$id\n";
...
#		print SP "$Doc2Id{$p1}\t$Doc2Id{$p2}\t", sprintf("%1.6f", $c/$max), "\n";
	my $pair = ($Doc2Id{$p1} lt $Doc2Id{$p2})?  # 2008/06/17
...
sub CreateTitleFile {
	...
	my($DBH, $STH, $sql, $ti, $encoding_name);
	$DBH = &InitDBH($Odsn, $main::ODB);
	...
	while (<F>) {
		...
		$encoding_name = Encode::Detect::Detector::detect($ti);
		if ($encoding_name =~ /UTF-8/i) { # if utf8-encoded
#			$ti = encode("big5", $ti); # from utf-8 to big5
#		if ($encoding_name !~ /big5/i) { # if utf8-encoded
...
sub CreateIndex_for_Clustering {
	...
	my($percent, $fn, $encoding_name, $txt); $Nnr = 0;
	$DBH = &InitDBH($Odsn, $main::ODB);
...
# remove those that are not journal artiles (NJA) # 2011/06/29
#  my @CiteRef = ();
#  foreach $cr (@$rCR) { next if $cr !~ /V\d+, P\d+/; push @CiteRef, $cr; }
#  @$rCR = @CiteRef;
...
# Now find the coupling (or co-citing) documents, given %CitedBy, %NR
	...
	my($cited, @P, $c, $i, $j, @T, $p1, $p2, $maxDF);
	$maxDF = ($main::OmaxDF>0)?$main::OmaxDF:($idn+1); # 2011/06/15
	$n = $percent = 0;
	...
	while (($p1, $p2) = each %CitedBy) {
		...
		next if @P > $maxDF; # 2011/06/15
		$c ++;
		for($i=0; $i<@P; $i++) { # for those have common citations
			for($j=$i+1; $j<@P; $j++) {
#				@T = sort ($P[$i], $P[$j]);
				my $tmp = ($P[$i] lt $P[$j])?"$P[$i]\t$P[$j]":"$P[$j]\t$P[$i]"; # 2008/06/17
...
sub CreateIndex_for_Multi_Stage_Clustering {
	...
	my($DBH, $cr, @CR, $Nnr, $n, $nn, $idn, $pro, $rDC, $encoding_name);
...
###			$sql = "SELECT UT FROM $table where CPI=$id"; 
		...
		$STH->execute()
		   or die "Can't run SQL statement: SQL=$sql, $DBI::errstr\n";
		while (($UT) = $STH->fetchrow_array) {
			$encoding_name = Encode::Detect::Detector::detect($UT);
			if ($encoding_name =~ /UTF-8/i) { # if utf8-encoded
#				$UT = encode("big5", $UT); # from utf-8 to big5
#			if ($encoding_name !~ /big5/i) { # if utf8-encoded
				from_to($UT, $encoding_name, 'big5'); 
			}
			if ($main::Opat) {
...
#		($ti, $did, $r) = split / : /, $_; 
		($ti, $did, $r) = split /\t:\t/, $_; 
#print STDERR "$ti, $did =>'$_'\n"; $text++; exit if $text > 3;
# Diapers having improved acquisition rates : 6392116 : Improved diaper cores containing particles of ...
...
#		next if length($ti) < 5 or length($did)<6 or length($did)>25; # not a valid line
		# The above line was replaced with the next line on 2010/12/04
#		next if length($r) < 5 or length($ti)<2 or length($did)>25; # not a valid line
		push @DocID, $did;
		$text .= $ti . ".\t";
