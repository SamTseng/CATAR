#!/usr/bin/perl -s
# File: mds.c, converted into Perl by Sam Tseng on 2018/01/14
# *
...
typedef struct { long int i;  double f;} VALUE;
VALUE   *value;

typedef struct { char *s; int   i; Boolean   defined; } LABEL;
LABEL    *label = NULL;
mdsTYPE  mdstype = mdsCLASSICAL;
Boolean  converged = false;
double   *x, *y, stress, *Tmat, final_value, final_r, magic = MAGIC, 
         final_magic, tol = TOL;

int arg_c, maxitdefined = 0, stopped_after;
long int size, k, maxit = MAXIT, niter = NITER, input_line = 0;
...
int main (int argc, char *argv []) {
    long int i, j, n, ierr, li, lsize, *ord, *orderord;
    double d, f, sum, *dbl_n, *fv1, *fv2, *vectors;
    char *infile;
    FILE *fp;
    LABEL *lp;
    no_mem_buffer = (char *) malloc (1024);
    get_programname (argv [0]);
    if (argc == 1 && isatty (fileno (stdin))) syntax (0);
    arg_c = argc;
    arg_v = argv;
    process_args ();
    switch (mdstype) {
    case mdsCLASSICAL:
      if (maxitdefined) errit ("Option -n not valid without option -K or -S");
      if (initfile) errit ("Option -i not valid without option -K or -S");
      break;
    case mdsKRUSKAL:
      if (maxit < 0) errit ("Illegal number of iterations");
      break;
    case mdsSAMMON:
      if (niter < 0) errit ("Illegal number of iterations");
      break;
    }
    ...
    switch (arg_c) {
    case 1:
      if (isatty (fileno (stdin))) syntax (1);
      fp = stdin;
    ...
    case 2:
      ...
      fp = fopen (infile, "r");
      if (! fp) errit ("Opening file \"%s\": %s", infile, strerror (errno));
      break;
    default:
      syntax (1);
    }
    ...
    if (outfile) {
      ...
      if (! fp_out) errit ("Creating file \"%s\": %s", outfile, strerror (errno));
    } else fp_out = stdout;
    print_head ();
    GetLine (fp, 1, infile);
    ...
    if (! initfile) Tmat = (double *) s_malloc (size * size * sizeof (double));
    for (i = 0; i < size; i++) {
      x [i + i * size] = 0.0;
      if (! initfile) Tmat [i + i * size] = 0.0;
       for (j = 0; j < i; j++) {
           GetLine (fp, 1, infile);
           if (sscanf (buffer, "%lf", &d) != 1)
             errit ("file \"%s\", line %li\nValue expected", infile, input_line);
          if (d <= 0.0 && (mdstype == mdsKRUSKAL || mdstype == mdsSAMMON))
            errit ("zero or negative distance between \"%s\" and \"%s\"", 
                    label [j].s, label [i].s);
          x [i + j * size] = x [i * size + j] = d;
          if (! initfile) Tmat [i + j * size] = Tmat [i * size + j] = d * d;
      }
    }
    if (fp != stdin) fclose (fp);
    if (k < 1 || k > size) errit ("invalid number of dimensions");
    if (mdstype != mdsCLASSICAL)
      for (i = 0; i < size; i++)
        for (j = 0; j < i; j++)
          if (x [i + j * size] <= 0)
            errit("zero or negative distance between objects \"%s\" and \"%s\"", label [i], label [j] );
    if (initfile) {
      ...
      GetLine (fp, 1, infile);
      if (atoi (buffer) != k) errit ("Illegal dimension in \"%s\", line %li", infile, input_line);
      while (GetLine (fp, 0, infile)) {
        lp = (LABEL *) bsearch (buffer, label, size, sizeof (LABEL), cmp_lbl);
        if (! lp) errit ("Unknown label \"%s\" in \"%s\", line %li", buffer, infile, input_line);
        lp->defined = true;
        for (j = 0; j < k; j++) {
          GetLine (fp, 1, infile);
      ...
      for (i = 0; i < size; i++) {
        sum = 0;
        for (j = 0; j < size; j++) sum += Tmat [i + j * size];
        sum /= size;
        for (j = 0; j < size; j++) Tmat [i + j * size] -= sum;
      }
      for (j = 0; j < size; j++) {
        sum = 0;
        for (i = 0; i < size; i++) sum += Tmat [i + j * size];
        sum /= size;
        for (i = 0; i < size; i++) Tmat [i + j * size] -= sum;
      }
      for (i = 0; i < size; i++)
        for (j = 0; j < size; j++) Tmat [i + j * size] *= -0.5;
      ...
      for (i = 0; i < k; i++) value [i].f = sqrt (value [i].f);
      y = (double *) s_malloc (size * k * sizeof (double));
      for (i = 0; i < size; i++)
        for (j = 0; j < k; j++)
          y [i + size * j] = vectors [i + value [j].i * size] * value [j].f;
      corr ();
      if (mdstype == mdsCLASSICAL) { print_results (); return 0; }
      free (value); free (vectors);
    }
    if (mdstype == mdsKRUSKAL) {
      ...
      for (i = 0; i < size; i++)
        ...
        for (i = 0; i < n; i++) ord [i] = value [i].i;
        free (value);
        orderord = (long int *) s_malloc ((size * size - size) / 2 * sizeof (long int));
        for (i = 0; i < n; i++) orderord [ord [i]] = i;
...
void checkCancel () {
    if (access ("_CANCEL_.L04", F_OK) == 0) {  errit ("CANCELLED"); }
}
...
void print_head () {
    fprintf(fp_out,
      ...
      "# Method: %s\n"
      "# Dimensions: %li\n",
      programname, sourcefile, method, k
    );
    if (mdstype == mdsKRUSKAL || mdstype == mdsSAMMON) {
    if (initfile) fprintf (fp_out, "# Initialised from: %s\n", initfile);
    }
}
...
void print_results () {
    ...
    for (i = 0; i < size; i++) {
      fprintf (fp_out, "%s\n", label [i].s);
      for (j = 0; j < k; j++) fprintf (fp_out, "% .8g\n", y [i + size * j]);
    }
...
void corr () {
    long int i, j, n;
    double f, f1, *yy, sumx, sumxx, sumy, sumyy, sumxy, sx, sy;
...
char *get_arg () {
    if (arg_v [1][2]) return arg_v [1] + 2;
    if (arg_c == 2) errit ("Missing argument for '%s'", arg_v [1]);
    arg_v++;
    arg_c--;
    return arg_v [1];
}
...
int order (void const *p1, void const *p2) {
    if (((VALUE *) p1)->f < ((VALUE *) p2)->f) return -1;
    if (((VALUE *) p1)->f > ((VALUE *) p2)->f) return 1;
    return 0;
}
...
# */
{
    ...
    for (;;) {
      ...
      input_line++;
      i = strlen (buffer);
      while (i && isspace ((unsigned char) buffer [i - 1])) buffer [--i] = '\0';
      i = 0;
...
void get_programname (char const *argv0) {
    ...
    if (p) programname = strdup (p + 1);
    else programname = strdup (argv0);
#endif
}
...
long int VR_n, VR_nr, VR_nc, VR_dimx, *VR_ord, *VR_ord2;
double *VR_d, *VR_y, *VR_yf, *VR_x;
...
void calc_dist (double *x) {
    ...
    for (index = 0; index < VR_n; index++) VR_y[index] = VR_d[VR_ord[index]];
}
    ...
    for (u = 0; u < r; u++) {
      for (i = 0; i < ncol; i++) {
        ...
        for (s = 0; s < r; s++) {
          ...
          if (s != u) {
            k = pd[k - 1];
            tmp += ((y[k] - yf[k]) / sstar - y[k] / tstar) * (x[u + r * i] - x[s + r * i]) / y[k];
          }
        }
        der[u + i * r] = tmp * ssq;
      }
...
double fminfn (double *x)v{
    double ssq;
    long int do_derivatives = 0;
    calc_dist (x);
...
void vmmin (long int n, double *b) {
    ...
    { # Begin of a block, for the following variables
  Fmin = f;
...
#   /* Resets unless has just been reset */
    ...
    free (g); free (t); free (X); free (c);
    free_Lmatrix(B, n);
}

void VR_sammon(double *dd, long int *nn, long int *kd, 
  double *Y, long int *niter, double *stress, double *aa, double *tol)
{
    int   i, j, k, m, n, nd;
    double *xu, *xv, *e1, *e2;
    double dpj, dq, dr, dt;
...
#    /* Error in distances */
    ...
    for (j = 1; j < n; j++) {
      ...
      for (k = 0; k < j; k++) {
        d = dd[k * n + j];
        if (d <= 0.0) errit ("some distance is zero or negative");
        tot += d;
        d1 = 0.0;
        for (m = 0; m < nd; m++) {
          xd = Y[j + m * n] - Y[k + m * n];
...
#          /* Correction */
        for (m = 0; m < nd; m++) xu[j + m * n] = Y[j + m * n] + magic * e1[m] / fabs(e2[m]);
      }
...
#        /* Move the centroid to origin and update */
      for (m = 0; m < nd; m++) {
        xx = 0.0;
        for (j = 0; j < n; j++) xx += xu[j + m * n];
        xx /= n;
        for (j = 0; j < n; j++) Y[j + m * n] = xu[j + m * n] - xx;
      }
      if (i % 10 == 0) {
        printf2 ("stress after %3d iters: %7.5f, magic = %5.3f\n", i, e, magic);
        if (e > epast - *tol) break;
        epast = e;
      }
    }
    if (i <= *niter) converged = true;
    else stopped_after = *niter;
    final_magic = magic;
    *stress = e;
...
#/* Subroutine */ 
int tql2_(integer *nm, integer *n, doublereal *d__, 
...
#/* Subroutine */ 
int tred2_(integer *nm, integer *n, doublereal *a, 
...
#/* Subroutine */ 
int rs_(integer *nm, integer *n, doublereal *a, doublereal *
...
#    /* Local variables */
  extern #/* Subroutine */ 
  int 
  tred1_(integer *, integer *, doublereal *, doublereal *, doublereal *, doublereal *), 
  tred2_(integer *, integer *, doublereal *, doublereal *, doublereal *, doublereal *), 
  tqlrat_(integer *, doublereal *, doublereal *, integer *), 
  tql2_(integer *, integer *, doublereal *, doublereal *, doublereal *, integer *);
