...
# perl -s ISI.pl -OSFtmDoc -OminDF=2 -ODBID=ID ItaA TSeg "DName, Desc" ..\Result\ItaA\ItaA_key_rt.txt > ..\Result\ItaA\ItaA_key_rt_DocList.txt
...
# Global variables :
	...
	my (%UsefulFields, @UsefulFieldIndex, $AU_index, $C1_index, $AB_index);
	# add the field: 'IN': the institute of the authors on 2009/01/30
...
#print STDERR join(", ", @UsefulFields), "\n"; exit;
	$AU_index = 0; # the index is for the above, $UsefulFields{AU} = $AU_index
	$AB_index = 6; # the index is for the above, $UsefulFields{AB} = $AB_index
	$C1_index = 7; # the index is for the above, $UsefulFields{C1} = $C1_index
...
#print STDERR "$f\n";
		...
		if ($line =~ /\tUT$/) { # tab-delimited format
			$TabDelimited = 1;
			($AU_index, $C1_index, $AB_index, @UsefulFieldIndex) 
...
#	 return var.: @UsefulFieldIndex, $C1_index, $AB_index, $AU_index
sub SetFieldIndex { # given the line of fields
	...
	for($i=0, $j=0; $i<@Fields; $i++) { # $i : the index of all fields from input
		if (defined $UsefulFields{$Fields[$i]}) { # $j : the index of useful fields
			...
			$C1_index = $j if $Fields[$i] eq 'C1';
			$AB_index = $j if $Fields[$i] eq 'AB';
			$AU_index = $j if $Fields[$i] eq 'AU';
...
print STDERR "\@UsefulFieldIndex=@UsefulFieldIndex\n";
	...
	return ($AU_index, $C1_index, $AB_index, @UsefulFieldIndex);
}

# Convert the data in some fields of @D to standard form and grow @D with IN, DP
#   change @D, %Country
# Use global $AU_index, $C1_index, $AB_index
sub StandardizeFields {
	my($rD, $rCountry) = @_;  
	my(@Addresses, %Cntry, $iw, @A, $addr, @IN, %IN, %DP, $d, @Au, $au);
...
#print "\$rD->[$AU_index]=$rD->[$AU_index]\n";
	$rD->[$AU_index] = &Normalize_Authors( $rD->[$AU_index] ); # authors
#print "\$rD->[$AU_index]=$rD->[$AU_index]\n";
# Now delete copyright information
#	$rD->[$AB_index] =~ s/\(C\) (19|20)\d\d *Elsevier .+ All rights reserved.\s*$//i;
	$rD->[$AB_index] =~ s/\(C\)\s*(19|20)\d\d\s*.+\s*(reserved|\w+)\.?\s*$//i;
	...
	$rD->[$AB_index] =~ s/\([^)]+(19|20)\d\d\)\s*$//;
	# end with FASEB J. 24, 158-172 ( 2010). www.fasebj.org
	$rD->[$AB_index] =~ s/^\s*(Context|BACKGROUND|Objectives):?\s*//i;
#print "AB=$rD->[$AB_index]\n";# if  $rD->[$UsefulFields{UT}] =~ /BIOABS:BACD200900471768/;

# Now get the countries of authors from field C1
#print STDERR "$Data[$C1_index]\n";
# From WoK:
#  C1 [Bell, Lindsay W.; Hargreaves, John N. G.] CSIRO Sustainable Ecosyst, APSRU, Toowoomba, Qld 4350, Australia.
#	 [Lawes, Roger A.; Robertson, Michael J.] CSIRO Sustainable Ecosyst, Po Wembley, WA 6913, Australia.
# From BioAbs
#  C1 SUMMERFIELD F W ; DEP FOOD SCI TECHNOL, UNIV CALIF, DAVIS, CA 95616,
#	 USA
#  C1 Ishigami, Akihito; Toho Univ, Dept Biochem, Fac Pharmaceut Sci, 2-2-1
#	 Miyama, Chiba 2748510, Japan
	$rD->[$C1_index] =~ s/\[[^\]]+\]//g; # delete parenthesized authors
#	@Addresses = split /;\s*/, $rD->[$C1_index]; # addresses of authors
	@Addresses = split /;\s*|\.\s*\n\s*/, $rD->[$C1_index]; # 2010/03/22
	@Addresses = ($rD->[$C1_index]) if $main::OBioAbs; # addresses of authors
...
#				$d = ucfirst lc $2;
			...
			@IN = split /,\s*/, $addr; # 2009/01/30
			$IN[0] =~ s/^\s*|\s*$//g; # 2010/05/04
			$IN[0] = join(' ', map {ucfirst lc} split(' ', $IN[0])); # 2011/04/04
			$IN{$IN[0]} += 1/($iw * $iw); # the first field is the institute name
#			$d = ucfirst lc $IN[1];
			$d = join(' ', map {ucfirst lc} split(' ', $IN[1]));
			$DP{$d} += 1/($iw * $iw); # the 2nd field is the Dept name
		}
#		@A = split /\s+|,/, $addr; 
		@A = split /,\s*/, $addr; 
		$addr = $A[$#A]; # last field is country
#print STDERR "$rD->[@$rD-1]\t$rD->[$C1_index]\n" if $addr =~ /\d/;
		...
		if ($addr =~ /ENGLAND|SCOTLAND|WALES/i) { $addr = 'UK'; }
		if ($addr =~ /CHINA/) { $addr = 'CHINA'; }# /PEOPLES R CHINA/
...
# For analysis of Asian countries in Science Education, last 2nd field is Hong Kong area
	...
	$rD->[$C1_index]=join("; ", sort {$Cntry{$b}<=>$Cntry{$a}} keys %Cntry);
# The next two fields will grow the length of @D
#	push @$rD, join("; ", sort {$IN{$b}<=>$IN{$a}} keys %IN); # 2009/01/30
	$rD->[@$rD]=join("; ", sort {$IN{$b}<=>$IN{$a}} keys %IN); #2010/05/10
#print STDERR "$rD->[$C1_index]\n";
...
# Now check if it exists before? If yes, skip, if no, write it out
		...
		&StandardizeFields(\@D, $rCountry); # grow @D with two fields: IN, DP
# write it out
#print STDERR "i=$i Fh=$Fh, => $_\n";
		if (not $main::Ocountry) {
			print "$CPI\t" if $main::OCPI;
...
#		chomp; @F = split /\t/, $line, scalar @Field; # 2011/11/10
		...
		if (@F < @Field) { 
			if ($line =~ /\t\t$/) { # the last two fields are empty
				push @F, "", ""; # @F<@Field if IN and DP is empty
...
# Update each field in the record with UT
	for ($i=0; $i<@$rF; $i++) {
		...
		next if $rF->[$i] eq '';
		$f = ($rField->[$i]=~/IN|ID/)?"[$rField->[$i]]":$rField->[$i];
		$sql = "UPDATE $Table Set $f = ? WHERE UT = ?";
		$sth = $DBH->prepare($sql);
		if ($f =~ /AB|CR/) {
			$sth->bind_param(1, $rF->[$i], SQL_LONGVARCHAR);
		} else {
...
sub Insert2DB_old { # did not work
	my($DBH, $Table, $rField, $rF) = @_;
	my($sql, $sth, $i, $str);
	$str = '?,' x (scalar @$rField); 
...
# Before insertion, delete the records in $Table
	...
	print STDERR "Failed to delete: $sql" if ($@);
# Now read records and insert into $Table
	$sql = "INSERT INTO $TSeg (SNO, Dname, [Desc]) "
	. "SELECT UT AS SNO, PY & ':' & TI AS Dname, AB AS [Desc] FROM $Table";
	$sth = $DBH->prepare($sql);
	eval { $sth->execute() or die $DBH->errstr; };
...
print STDERR "Total records: $N\n";
	...
	my $sql = "SELECT UT, AU, TI, AB, DE, ID, SC, NR, TC, CR, C1, [IN], DP FROM $Table";
#	my $sql = "SELECT UT, PY, TI, AB, DE, ID, SC FROM $Table where DE like 'biomass recovery%'";
	my $STH = $DBH->prepare($sql)
		   or die "Can't prepare SQL statement: SQL=$sql, $DBI::errstr\n";
	$STH->execute()
		   or die "Can't run SQL statement: SQL=$sql, $STH::errstr\n";
	$n = 0;
	my($c, @AU, @TI, @AB, @DE, @ID, @SC, @NR, @TC, @CR, @C1, @IN, @DP, @C);
	foreach $c (@AU[0..3], @TI[0..3], @AB[0..3], @DE[0..3], @ID[0..3], 
	@SC[0..3], @NR[0..3], @TC[0..3], @C1[0..3], @IN[0..3], @DP[0..3]) { $c=0; }
	$AU[1] = $TI[1] = $AB[1] = $DE[1] = $ID[1] = $SC[1] = $NR[1] 
		= $TC[1] = $C1[1] = $IN[1] = $DP[1] = 100000;
...
# index 0: sum of word counts, 1: min of word count, 2: max, 3: empty record count
		...
		$c = @C = split /;\s*/, $in;
		$IN[0] += $c;   $IN[1] = $c  if $c < $IN[1] and $c>0;
		$IN[2]  = $c if $IN[2] < $c; $IN[3]++ if $c == 0;
		$c = @C = split /;\s*/, $dp;
	...
	print "TI: EmptyRec=$TI[3], MaxWordCount=$TI[2], MinWordCount=$TI[1], Avg=", &ts($TI[0]/$N), "\n",
		  ...
		  "IN: ZeroRec=$IN[3], Max=$IN[2], Min=$IN[1], Avg=", &ts($IN[0]/$N), "\n",
...
#	my $sql = "SELECT UT, AU, TI, AB, C1, [IN], DP, MC, CC, TA, DS, CH, MQ, PR, MI FROM $Table";
	my $sql = "SELECT UT, AU, TI, AB, C1, [IN], DP, MC, CC, MI FROM $Table";
#	my $sql = "SELECT UT, PY, TI, AB, DE, ID, SC FROM $Table where DE like 'biomass recovery%'";
	my $STH = $DBH->prepare($sql)
		   or die "Can't prepare SQL statement: SQL=$sql, $DBI::errstr\n";
	$STH->execute()
