...
# 
    ...
    my $stime = time();
    &InitWG(); # set $me
# perl -s term_cluster.pl -Oidx='eg. wgIdxName' -Oindex arg_idx_words.txt
...
sub DumpIndexTerms {
    my($IdxWordFile) = @_; my($stime, $n, $iid);
    $stime = time();
    $iid = &IndexName2ID($me->{'wg'}, $main::Oidx); # 2017/05/04
#    $n = $seg->DumpWGindexWords(0, 'WGindexWords.txt');
#    $n = &DumpWGindexWords(0, 'WGindexWords.txt');
    $n = &DumpWGindexWords($iid, $IdxWordFile); # 2017/05/04
...
sub GetKeyword_TimeSeries {
    ...
    my($id, $kw, $kwdf, $rest, @DateSeries, $rDid2PK, $rDid2Title, @YearMonths, @Months);
    @DateSeries = split ' ', $DateSeries;
#    @Months = ('01', '02', '03', '04', '05', '06', '07', '08', '09', '10', '11', '12');
#    foreach my $yn (@DateSeries){
#      foreach my $mn (@Months){
#        unshift @YearMonths, $yn+$mn;
#      }
#    }
#    @DateSeries = grep{length($_)>4} @YearMonths;
#    @DateSeries = ('200001','200002','200003','200004','200005','200006','200007','200008','200009','200010','200011','200012',
...
sub GetDoc_Date_PK_Title { # Okey with WG 2.8 and WG 3.57, 2007/01/30
    my($wg, $Iid, $IdxName) = @_;  
    my(@Did2Date, $N, $I, $J, $Did, $date, $year, $month, $day, $stime, $etime, $dte);
    my(@Did2PK, @Did2Title, $RN);
    $stime = time();
...
#  if (1) { warn "use GetDocFieldContent()\n"; # test GetDocFieldContent() under WG 3.57
    ...
    for ($I=0, $J=0; $I < $N; $I++) {
        $Did = $wg->GetDocID($I);  # 取得文件ID
	$dte = $date = $wg->GetBatchDocFieldContent($J++);
...
#print STDERR "Did=$Did, date=$date, dte=$dte\n"; exit;
        $Did2Date[$Did] = $date;
    }
...
sub Init {
    ...
    $rDC = Cluster->new( { 'debug'=>1 } ); # original value is 1 2011/09/28
    ...
    $rDC->SetValue('Sim', $main::Sim) if $main::Sim; # e.g. 'PureDice' 2009/11/27 original sim = $main::Sim 2011/09/28
...
print STDERR "  It takes ", time() - $sot, " seconds to insert $rtn (out of $tfn) documents\n" if $main::Odebug;
    ...
    $rDC->SetValue('Method', 'CompleteLink'); # 'SingleLink', 'Cluto', or 'SOM' # original method is CompleteLink 2011/09/27
    $rDC->SetValue("low_df", 0); # do not use term whose df <= 2 # original value = 0 that diff from comment 2011/09/26
