#!/usr/bin/perl -s
    ...
    use strict; use vars;
    use lib "DbXls"; # You need to specify the path of module: 'DbXls.pm'
    use DbXls;
...
# C:\CATAR_src\src\Perl_Module\eport>copy \CATAR_src\src\Paper_org.mdb eport.mdb
# C:\CATAR_src\src\Perl_Module\eport>perl -s InsertCR.pl -Oxls -Odebug C:\CATAR_src\src\Perl_Module\eport\bib.xls C:\CATAR_src\src\Perl_Module\eport\ref.xls eport.mdb
# Note: the excel file must be specified in absolute path file name! # 2011/11/23
...
#    my $path = 'C:/CATAR_src/src/Perl_Module/eport/';
...
#    $Book = $Excel->Workbooks->Open($path . $refFile);
    $Book = $Excel->Workbooks->Open($refFile); # use absolute file path, or error
...
#  $Book = $Excel->Workbooks->Open($path . $bibFile);
  $Book = $Excel->Workbooks->Open($bibFile); # use absolute file path, or error
	$Sheet = $Book->Worksheets(1);  # open the default sheet, normally names 'Sheet1'
	...
	foreach my $row (3..100000) {
	    ...
	    if ($CR eq '') { $CR = ''; } else { $CR = join("; ", @{$CR[$CR]}); }
